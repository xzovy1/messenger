...
describe("Message Router", () => {
    ...
    test("new conversation fails with incorrect recipient", async () => {
        ...
        const chat = await request(app).post("/api/chat").set("authorization", `bearer ${session.jwt}`).set("Content-Type", 'application/json').send({ to: badId })
        expect(chat.body.message).toBe(`unable to create conversation with user id ${badId}`)
    })
